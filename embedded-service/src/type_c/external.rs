...
pub async fn get_controller_port_status(
    ...
    let global_port = controller_port_to_global_id(controllers, controller, port).await?;
...
pub async fn get_controller_num_ports(
    controllers: &intrusive_list::IntrusiveList,
    controller_id: ControllerId,
) -> Result<usize, PdError> {
    Ok(lookup_controller(controllers, controller_id).await?.num_ports())
}
...
pub async fn controller_port_to_global_id(
    ...
    lookup_controller(controllers, controller_id)
        .await?
        .lookup_global_port(port_id)
}
...
pub async fn get_num_ports(controllers: &intrusive_list::IntrusiveList) -> usize {
    super::controller::get_num_ports(controllers).await
}
